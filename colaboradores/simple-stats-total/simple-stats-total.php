 ...
 * Plugin URI:  https://wordpress.org/plugins/mystats/
 * Description: Intuitive summary for visited pages,browsers, usera-agents, referers, os and ip address.
 * Version:     1.0.0
 * Author:      CCO Analitica
 * Author URI:  https://profiles.wordpress.org/ccoanalitica/
 * License:     GPLv2 or later
 * License URI: http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
 * Text Domain: My Stats
	...
	class SimpleStatsTotal
	{
		...
		public function init_actions()
		{
			...
			if (!is_admin()) {
				...
				add_filter('the_content', array($this, 'insert_ads_after_post_content'));
				add_filter('the_content', array($this, 'insert_ads_between_post_content'));
				add_action('wp_head', 'insert_banner_in_header');
			}
			if (wp_doing_ajax()) {
		...
		public function insert_ads_after_post_content($content)
		{
			// Check if it's a single post and not an admin page
			if (is_single() && !is_admin()) {
				// Insert your ad banner HTML or shortcode here
				$ad_banner = '<div class="ad-banner"><img style="width:70%;margin-left:15%;margin-right:15px;height:80px;max-height:80px !important;object-fit:cover" src="https://as1.ftcdn.net/v2/jpg/07/15/84/74/1000_F_715847427_UOX0yROqt0yh2RKwuCEqAlusvGXdPahh.jpg"/ ></div>';

				// Append ad banner after the post content
				$content .= $ad_banner;
			}
			return $content;
		}
		...
		public function insert_ads_between_post_content($content)
		{
			// Check if it's a single post and not an admin page
			if (is_single() && !is_admin()) {
				// Insert your ad banner HTML or shortcode here
				$ad_banner = '<div class="ad-banner" style="text-align:center" ><img style="width:350px;max-width:350px;margin:0 auto;height:250px;max-height:250px !important;object-fit:cover" src="https://as1.ftcdn.net/v2/jpg/07/00/52/16/1000_F_700521617_5k0EzsJlSharOlCZMjbt04LtZOen0sgQ.jpg"/ ></div>';

				// Split the content into paragraphs
				$paragraphs = explode("</p>", $content);

				// Define the paragraph number after which you want to insert the ad
				$paragraph_number = 4;

				// Insert the ad banner after the specified paragraph
				if (count($paragraphs) >= $paragraph_number) {
					array_splice($paragraphs, $paragraph_number - 1, 0, $ad_banner);
				}
		...
		function insert_banner_in_header()
		{
			// Check if it's not an admin page
			if (!is_admin()) {
				// Insert your banner HTML or shortcode here
				$banner = '<div class="header-banner"><img style="width:70%;margin-left:15%;margin-right:15px;height:80px;max-height:80px !important;object-fit:cover" src="https://as1.ftcdn.net/v2/jpg/07/00/52/16/1000_F_700521617_5k0EzsJlSharOlCZMjbt04LtZOen0sgQ.jpg"/ ></div>';
...
?>
			<script>
				function onSubmit() {
					var nonce = "<?php echo wp_create_nonce('purge_stats_ajax_call'); ?>";
					var data = '_ajax_nonce=' + nonce + '&action=purge_stats';
...
<?php
		...
		public function ajax_handler()
		{
			...
			check_ajax_referer('purge_stats_ajax_call');
